-------------------------------------------------
-----Atomic 2021 RDM Magic script------
-----       v0.3 andybabin@me.com          ------ 
-------------------------------------------------
...
decoder_count = #uid_device_table --keeps count of number of discovered decoders
	...
	io.write('node ID? ') --asks for nodeid
...
function color_cycle() --does a red/green/blue cyle to test 
...
while i <= 4 do
	os.execute(ola_rdm_start .. box_table[i] .. ' dmx_personality 2')
	os.execute(ola_rdm_start .. box_table[i] .. ' dmx_start_address '.. start)
	--add a thing to name the nodes based on name/position-------------------------------------------------------------------------------------- 
...
file = io.open(database, "a")
 
file:write("\n",node_id ..","..box_table[1]..","..box_table[2]..","..box_table[3]..","..box_table[4]..",,,,,")
 
file:close()

os.execute(ola_dmx_out .. "255,255,0,0,0,0,0,0,255,255,0,0,0,0,0,0,255,255,255,255,255,255") --makes first to ch red


--write this out to a file, box table with ID 
